...
A fully interactive two-player Battleship game built in C++ for the terminal.
This project implements ship placement, rotation, attack logic, win detection, and persistent game history logging.
...
Ships are hidden from opponent’s board view

Type:

Es

to exit the match early.

🏆 Win Condition

A player wins when all opponent ship cells are destroyed.
...
You can view past matches from the main menu.
...
Better UI abstraction

Cross-platform color handling

Refactoring large functions into smaller modules

Removing goto statements for cleaner control flow
